...
srand(time(NULL)); //Seed for random numbers
 ...
 numbers = fopen("numbers_fft4.txt", "r");
 results = fopen("results_eh_4p.txt", "r"); 
 bin_file = fopen("binary_FFT4point.txt", "w");
 log_file = fopen("LogFile_fft4p.txt", "w");
 results_initial = fopen("results_dsp_fft4p.txt", "w");
...
for (i = 0; i < Num_Iterations; i++)
  ...
  fprintf(log_file, "Sent A0,Sent B0,Sent A1,Sent B1,,Read X0, Read X1,Read X2,Read X3,,Exp X0,Exp X1,Exp X2,Exp x3 \n"); 
