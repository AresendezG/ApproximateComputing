...
#define Num_Iterations 25000 //Number of total iterations of the experiment

#define filename_inform "AXA8_PA_rightside-Multiplier-inform.txt"
#define filename_results "AXA8_PA_rightside-Multiplier-results.txt"
#define filename_binary "AXA8_PA_rightside-Multiplier-binary.txt"

int Error_Place [N_bits]; //Counts how many errors in each bit overall
int Error [16][16]; //Errors [How_Many_Wrong] [Which_Position]
...
void BinaryConversion(int DSP_read, int Exact, int numBits){
	...
	//if(WrongBits>0) //Only if there are more than 1 wrong bit
		for (i=0; i<16; i++){ //Checks which of the bits doesn't match
...
int main(void){
 ...
 /*File management*/
   ...
   results_file = fopen(filename_results, "w");
	   bin_file = fopen(filename_binary, "w");
    inform_file = fopen(filename_inform,"w"); 
