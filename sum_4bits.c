...
#define N_bits 4 //This is the width of the processor 

#define Num_Iterations 5000 //Number of total iterations of the experiment
...
void BinaryConversion(int DSP_read, int Exact, int numBits){
	...
	if(WrongBits>0) //Only if there are more than 1 wrong bit
		for (i=0; i<N_bits; i++) //Checks which of the bits doesn't match
		  ...
		  Error[WrongBits][i]++;  //Errors[How Many?] [At which position?]
			/*Since we have how many bits were wrong, we can spot in which positions */
			/*Needs to be printed*/
}
...
fprintf(bin_file, "DSP Binary Read,Binary Exact,Wrong Bits,Match \n");
fprintf(results_file, "A,B,Result,DSP Read,Match,ABS Dist \n");
...
for(i=0; i<N_bits+1; i++)
	for(j=0;j<N_bits+1; j++)
		Error[i][j]=0;
for (A=0; A<16; A++){
for(B=0; B<16; B++) //Main Loop
{
//	A = randint(15);
//	B = randint(15);
	reset();
	...
	fprintf(results_file, "%i, %i, %i, %i, %c, %i \n", A, B, (A+B), DSP, m, abs((A+B)-DSP));
...
for (i=0; i<N_bits; i++)
  fprintf(inform_file, "Bit Error in %i:, %i, \n",i,Error_Place[i]); 

fprintf(inform_file,"\n Wrong Bit Counter,Bit 0,Bit 1,Bit 2,Bit 3 \n");
